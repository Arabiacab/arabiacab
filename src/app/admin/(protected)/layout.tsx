...
const navItems = [
  ...
  { href: '/admin/drivers',       label: 'Drivers',       icon: 'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4' },
  ...
  return (
    <ToastProvider>
      <div className="flex h-screen overflow-hidden bg-[#0A0A0A]">
        ...
        {/* Sidebar */}
        <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-[#111111] border-r border-white/[0.08] flex flex-col transition-transform duration-200 lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            {navItems.map(item => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setSidebarOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${active ? 'bg-[#CCFF00]/10 text-[#CCFF00]' : 'text-white/50 hover:bg-[#1A1A1A] hover:text-white'}`}
                >
                  ...
                  {isNotif && unreadCount > 0 && (
                    <span className="ml-auto bg-[#CCFF00] text-[#0A0A0A] text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{unreadCount}</span>
                  )}
                </Link>
              );
            })}
          ...
          <div className="p-3 border-t border-white/[0.08]">
            <Link
              href="/"
              target="_blank"
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-white/50 hover:bg-[#1A1A1A] hover:text-white transition-colors mb-1"
            >
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-white/50 hover:bg-red-500/10 hover:text-red-400 transition-colors"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Top bar */}
          <header className="h-14 bg-[#111111] border-b border-white/[0.08] flex items-center px-4 gap-4 flex-shrink-0">
            <button
              onClick={() => setSidebarOpen(true)}
              className="lg:hidden p-2 rounded-lg text-white/50 hover:text-white hover:bg-[#1A1A1A] transition-colors"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="ml-auto flex items-center gap-3">
              <Link href="/admin/notifications" className="relative p-2 rounded-lg text-white/50 hover:text-white hover:bg-[#1A1A1A] transition-colors">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="w-8 h-8 rounded-full bg-[#CCFF00] flex items-center justify-center text-[#0A0A0A] font-bold text-sm">A</div>
            </div>
          </header>
